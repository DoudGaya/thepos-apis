/**
 * eBills.Africa Vendor Adapter
 * 
 * Implementation of VendorAdapter interface for eBills.Africa API
 * Base URL: https://ebills.africa/wp-json
 * Auth: JWT Bearer Token (7-day expiry)
 * 
 * NOTE: eBills.Africa API is 100% compatible with VTU.NG API
 * Same endpoints, same request/response format, same authentication
 */
...
export class EBillsAdapter implements VendorAdapter {
  ...
  /**
   * Authenticate with eBills.Africa and obtain JWT token
  ...
  async authenticate(): Promise<void> {
    ...
    try {
      ...
      console.log('[eBills.Africa] Authentication successful')
    } catch (error: any) {
      console.error('[eBills.Africa] Authentication failed:', error.message)
      throw new VendorError(
        'eBills.Africa authentication failed',
        'EBILLS',
  ...
  private getAuthHeaders() {
    if (!this.token) {
      throw new Error('eBills.Africa: Not authenticated')
    }
  ...
  async getPlans(service: ServiceType, network?: NetworkType): Promise<ServicePlan[]> {
    ...
    throw new Error(`eBills.Africa: getPlans not implemented for ${service}`)
  }
  ...
  async buyService(payload: PurchasePayload): Promise<VendorPurchaseResponse> {
    ...
    try {
      ...
      switch (payload.service) {
        ...
        default:
          throw new Error(`eBills.Africa: Service ${payload.service} not implemented`)
      }
  ...
  /**
   * Map eBills.Africa status strings to our standard status enum
   */
  ...
  private handleError(error: any): never {
    ...
    if (status >= 500) {
      throw new VendorError(
        'eBills.Africa service temporarily unavailable',
        'EBILLS',
        status,
        data
      )
    ...
    throw new VendorError(
      error.message || 'Unknown eBills.Africa error',
      'EBILLS',
      500,
      data
    )
